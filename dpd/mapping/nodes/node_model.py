...
class NodeModel(Model):
    ...
    def object_for_node(self, node):
        ...
        if node.get("type") == "stop_sign":
            object = StopSign(model=self, unique_id=unique_id, geometry=node["geometry"])
        elif node.get("type") == "yield_sign":
            object = YieldSign(model=self, unique_id=unique_id, geometry=node["geometry"])
        elif node.get("type") == "stop_light":
            object = StopLight(model=self, unique_id=unique_id, geometry=node["geometry"])
        elif node.get("type") == "stop":
            object = Stop(model=self, unique_id=unique_id, geometry=node["geometry"], dwell_time=node["dwell_time"], entry_velocity = 0 * self.velocity_unit)
        else:
