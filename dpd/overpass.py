...
def elements2ways(elements, nodes):
    ...
    for element in elements:
        if element["type"] == "way":
            ways[element["id"]] = LineString(
                [(nodes[nodeid]["geometry"].coords[0]) for nodeid in element["nodes"]]
            )
    return ways
        ...
        if element["type"] == "relation":
            if element["tags"]["type"] == "boundary":
                multilinestring = MultiLineString(
                    list(
                        filter(bool, map(partial(wayid2way, ways), element["members"]))
                    )
                )
                rels[element["id"]] = {}
...
def query2elements(
    query, element_type="rels", endpoint="http://overpass-api.de/api/interpreter"
):
    payload = {"data": query}
...
def get_key_value_in_area(
    area, key, value, endpoint="http://overpass-api.de/api/interpreter"
):
    query = """
    [out:json][timeout:60];
    area(%d)->.searchArea;
    ...
    for element in elements:
        if element["type"] == "way":
            ways[element["id"]] = {
                "geometry": LineString(
                    [
                        (nodes[nodeid]["geometry"].coords[0])
        ...
        if element["type"] == "relation":
            if element["tags"]["type"] == "boundary":
                ...
                relations[element["id"]] = {
                    "geometry": list(polygonize(multilinestring))[0],
                    "element": element,
                }
            else:
