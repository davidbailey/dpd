...
class EdgesDriver(Agent):
    def __init__(self, body, edges, initial_driver_position_offset=0, driver_max_velocity=None, driver_final_velocity=None, *args, **kwargs):
        """
        ...
        if driver_max_velocity == None and type(body) == KinematicBody:
            warn("driver_max_velocity is not set: Driver will not accelerate after slowing due to maxspeed. Try setting driver_max_velocity equal to initial_velocity")
        self.body = body
        self.edges = edges
    ...
    def begin_next_edge(self, extra_position):
        ...
        if self.driver_max_velocity is not None:
            self.body.max_velocity = minimum(self.current_edge.get("maxspeed", inf), self.driver_max_velocity)
        else:
