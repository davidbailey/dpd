...
class Vehicle:
    ...
    def accelerate_or_decelerate(self, distance, acceleration_or_deceleration):
        ...
        self.speed = np.sqrt(speed_before_segment**2 + 2 * acceleration_or_deceleration * distance)
    ...
    def accelerate_and_go(self, speed_limit, distance):
        accelerate_distance = (speed_limit**2 - self.speed**2) / (2 * self.acceleration)
    ...
    def accelerate_and_decelerate(self, speed, distance, speed_limit):
        deceleration_distance = (2 * self.acceleration * distance + speed**2 - speed_limit**2) / (2 * (self.acceleration - self.deceleration))
        self.accelerate_or_decelerate(distance - deceleration_distance, self.acceleration)
    ...
    def fix_overspeed(self, speed_limit, distance, fix_overspeed_distance=0):
        ...
        if speed_limit**2 > last_segment['speed_before_segment']**2 + 2 * self.deceleration * (fix_overspeed_distance):
            self.accelerate_and_decelerate(last_segment['speed_before_segment'], fix_overspeed_distance, speed_limit)
        else:
