class ABTMMap(Map):
    def __init__(self, model, map_):
        ...
        self.intersections["Intersection"] = self.intersections.apply(
            lambda intersection: self.transform_intersection_to_agent_based(intersection),
            axis=1
        )
        self.roads["Road"].map(
            lambda road: self.transform_road_to_agent_based(road)
        )
        self.clear_lanes()
    ...
    def transform_intersection_to_agent_based(self, intersection):
        if hasattr(intersection, "Type"):
            ...
            if intersection_type == "Signal":
                intersection = SignalIntersection(intersection["Intersection"], self.model)
            elif intersection_type == "Yield":
                intersection = StopIntersection(intersection["Intersection"], self.model)
            else:
    ...
    def transform_road_to_agent_based(self, road):
        ...
        if road.input_intersection:
            road.input_intersection = self.intersections.loc[road.input_intersection.name]["Intersection"]
        if road.output_intersection:
            road.output_intersection = self.intersections.loc[road.output_intersection.name]["Intersection"]
    ...
    def refresh_people_geometries(self):
        self.people["geometry"] = self.people["Person"].map(lambda person: person.geometry)
    ...
    def abtmplot(self, include_intersections=False, include_roads=False):
        ...
        self.plot(fig=fig, ax=ax, include_intersections=include_intersections, include_roads=include_roads)

    def abtmplot_folium(self, include_intersections=False, include_roads=False, fields_people=None):
        self.refresh_people_geometries()
        ...
        return self.plot_folium(folium_map=folium_map, include_intersections=include_intersections, include_roads=include_roads)
