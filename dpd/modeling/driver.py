...
class Driver(Agent):
    ...
    def step(self):
        ...
        if self.length_on_lane >= self.road.geometry.length:
            ...
            if person_in_front_of_me.length_on_lane < self.length_on_lane + self.stopping_distance:  # congestion! let's see if we can change lanes
                if self.road.lanes[self.lane.lane_number + 1]:
                    if self.attempt_lane_change(self.road.lanes[self.lane.lane_number + 1]): # returns false if lane change successful, true if unsuccessful
                        if self.road.lanes[self.lane.lane_number - 1]:
                            if not self.attempt_lane_change(self.road.lanes[self.lane.lane_number - 1]):
                                self.move_forward()
    ...
    def attempt_lane_change(self, lane):
        ...
        index_of_new_person_in_front_of_me = bisect.bisect_left(list(map(lambda x: x.length_on_lane, lane.occupants)), self.length_on_lane)
        index_of_new_person_behind_me = bisect.bisect_right(list(map(lambda x: x.length_on_lane, lane.occupants)), self.length_on_lane)
        if index_of_new_person_in_front_of_me > 0:
            new_person_in_front_of_me = lane[index_of_new_person_in_front_of_me]
            if not new_person_in_front_of_me > self.length_on_lane + self.stopping_distance:
                return True
        if index_of_new_person_behind_me < len(lane.occupants):
            new_person_behind_me = lane[index_of_new_person_behind_me]
            if not new_person_behind_me.length_on_lane + new_person_behind_me.stopping_distance < self.length_on_lane:
                return True
